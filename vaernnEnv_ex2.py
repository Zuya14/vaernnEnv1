...
import bullet_lidar
import robot_sim_ex2 as robot_sim  

class vaernnEnv_ex2(gym.Env):
    global_id = 0
    ...
    def setting(self, _id=-1, mode=p.DIRECT, sec=0.01):
        if _id == -1:
            self.sim = robot_sim.robot_sim(vaernnEnv_ex2.global_id, mode, sec)
            vaernnEnv_ex2.global_id += 1
        else:
    ...
    def copy(self, _id=-1):
        new_env = vaernnEnv_ex2()
        
        if _id == -1:
            new_env.sim = self.sim.copy(vaernnEnv_ex2.global_id)
            vaernnEnv_ex2.global_id += 1
        else:
