...
import bullet_lidar
import robot_sim3 as robot_sim  

class vaernnEnv3(gym.Env):
    global_id = 0
    ...
    def setting(self, _id=-1, mode=p.DIRECT, sec=0.01):
        if _id == -1:
            self.sim = robot_sim.robot_sim(vaernnEnv3.global_id, mode, sec)
            vaernnEnv3.global_id += 1
        else:
    ...
    def copy(self, _id=-1):
        ...
        if _id == -1:
            new_env.sim = self.sim.copy(vaernnEnv3.global_id)
            vaernnEnv3.global_id += 1
        else:
