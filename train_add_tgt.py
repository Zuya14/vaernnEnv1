...
from EpisodeMemory2 import Episode, EpisodeMemory
...
def collect_init_episode(memory_size, collect_num, min_step, clientReset=False, sample_rate=0.001, sec=0.01):
    ...
    while collect_count < collect_num:
        ...
        # for _ in range(max_step):
            ...
            episode.append(observation[:1080], action, reward, done, relativeTgt)
...
if __name__ == '__main__':
    ...
    if not args.no_init_train:
        for _ in range(args.threads):
            datas_observations, datas_actions, datas_rewards, datas_dones, next_observations, datas_tgts, next_datas_tgts = memory.sample(n=args.batch_size, L=args.chunk_size)
            ...
            latent_tgt  = torch.cat([z.view(args.batch_size, args.chunk_size, latent_size), datas_tgts.view(args.batch_size, args.chunk_size, 1).to(device)], dim=-1)
            latent_tgt2 = torch.cat([z2.view(args.batch_size, 1, latent_size), next_datas_tgts.view(args.batch_size, 1, 1).to(device)], dim=-1)

            rnn_loss, predicts, hiddens = rnn_train.train(latent_tgt, datas_actions, latent_tgt2)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        with torch.no_grad():
            ...
            relativeTgt = torch.tensor([env.getRelativeTgt(tgtPos)]).to(device)        
            ...
            old_states  = torch.cat([old_states, relativeTgt.view(1, 1, -1)], dim=-1)
            ...
            for i in range(1, max_step+1):
                ...
                episode.append(observation, env.sim.action, reward, done, relativeTgt)
        ...
        if epoch % (args.epochs//10) == 0:
            ...
            with torch.no_grad():
                datas_observations, datas_actions, datas_rewards, datas_dones, next_observations, datas_tgts, next_datas_tgts = memory.sample(n=args.test_batch_size, L=args.chunk_size+args.test_predict_step)

                obs = datas_observations.to(device)
                act = datas_actions.to(device)
                obs2 = next_observations.to(device)

                tgts  = datas_tgts.to(device)
                tgts2 = next_datas_tgts.to(device)
                ...
                latent  = torch.cat([ z.view(args.test_batch_size, args.chunk_size+args.test_predict_step, -1), tgts.view(args.test_batch_size, args.chunk_size+args.test_predict_step, -1)], dim=-1)
