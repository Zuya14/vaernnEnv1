...
import bullet_lidar
import robot_sim4 as robot_sim  

class vaernnEnv4(gym.Env):
    global_id = 0
    ...
    def setting(self, _id=-1, mode=p.DIRECT, sec=0.01):
        if _id == -1:
            self.sim = robot_sim.robot_sim(vaernnEnv4.global_id, mode, sec)
            vaernnEnv4.global_id += 1
        else:
    ...
    def copy(self, _id=-1):
        ...
        if _id == -1:
            new_env.sim = self.sim.copy(vaernnEnv4.global_id)
            vaernnEnv4.global_id += 1
        else:
