...
register(
    id='vaernn-v4',
    entry_point='vaernnEnv4:vaernnEnv4'
)
...
def collect_init_episode(memory_size, collect_num, min_step, clientReset=False, sample_rate=0.001, sec=0.01):
    ...
    while collect_count < collect_num:
        ...
        if y == 3:
            y = 4
        if y < 3:
            env.reset(clientReset=clientReset, dynamic_counter=dynamic_counter, interval=interval, x=random.randint(-1, 1)*1.25, y=y)
        else:
            env.reset(clientReset=clientReset, dynamic_counter=dynamic_counter, interval=interval, x=1.25*(-1**random.randint(0, 1)), y=y)
