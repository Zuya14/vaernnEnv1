...
def collect_init_episode(memory_size, collect_num, min_step, clientReset=False, sample_rate=0.001, sec=0.01):
    ...
    while collect_count < collect_num:
        ...
        if y <= 1:
            env.reset(clientReset=clientReset, dynamic_counter=dynamic_counter, interval=interval, x=random.randint(-1, 1)*0.75, y=y)
        else:
            env.reset(clientReset=clientReset, dynamic_counter=dynamic_counter, interval=interval, x=0.75*(-1**random.randint(0, 1)), y=y)
